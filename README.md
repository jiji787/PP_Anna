...
```bash
dotnet build репазиторий.csproj 
dotnet run --project репазиторий.csproj 
или 
dotnet run
```
...
**Что сделано:**
- В README добавлен раздел «Установка и запуск проекта» с пошаговыми инструкциями для нового разработчика.
- Создан файл `TROUBLESHOOTING.md` с описанием типовых ошибок и их решений (PostgreSQL, Kafka, Git, Docker, сборка).
- Добавлен раздел с командами для запуска консольного приложения, Web API, Docker-контейнеров и тестов.
- Проведена финальная проверка ссылок и структуры документации.

**Артефакты:**
- [TROUBLESHOOTING.md](TROUBLESHOOTING.md) – гайд по типовым ошибкам и их устранению.

**Результат:** Документация проекта теперь полная и понятная для любого нового разработчика.


### 01.07.2026 (ср)

**Что сделано:**
- Доработана презентация по итогам практики (добавлены скриншоты, диаграммы, результаты профилирования).
- Подготовлен текст выступления (15-20 минут) с акцентом на ключевые этапы: настройка среды, разработка CRUD, интеграция с Kafka/Redis/Vault, администрирование БД.

### 02.07.2026 (чт)

**Что сделано:**
- Проверена работа всех компонентов системы: консольное приложение (CRUD, отчёты), Web API (Swagger), Docker-контейнеры (Redis, Vault, Kafka).
- Проведена конференция демонстрации результатов производственной практики.
- Подготовлены ответы на возможные вопросы по архитектуре, выбору технологий, трудностям и их решениям.

### 03.07.2026 (пт)

**Что сделано:**
- Получены рекомендации по доработке документации (при наличии).
- Оформлены итоговые документы для сдачи: задание на практику, дневник, отчёт.
- Проведена финальная проверка всех артефактов в репозитории GitHub.


### 06.07.2026 (пн)

**Что сделано:**
- Распечатаны и подписаны все необходимые документы (задание, дневник, отчёт) в соответствии с требованиями.
- Собрана папка-скоросшиватель в правильном порядке (задание -> дневник -> отчёт).
- Документы сданы руководителю практики на проверку и подпись.

**Итог:** производственная практика полностью завершена, все документы сданы в срок.



==========

## Соответствие заданиям практики

### Задание на период 01.06 – 21.06 (разработка ПО, ПМ.02)

| № | Пункт задания | Где отражено |
|---|---------------|--------------|
| 1 | Участие в выработке требований к ПО | `requirements.md` (User Stories, DDD) |
| 2 | Владение основными методологиями разработки ПО | `requirements.md` (сравнение Waterfall, Agile, Scrum, Kanban) |
| 3 | Участие в проектировании ПО с использованием специализированных пакетов | C4-диаграммы, диаграммы последовательности и компонентов в README |
| 4 | Участие в разработке тестовых наборов и сценариев | `EfCoreDemo.Tests/UnitTests/UserRepositoryTests.cs` (8 тестов) |
| 5 | Использование методов для получения качественного кода | Рефакторинг (`UserService.cs`), статический анализ (`dotnet format`) |
| 6 | Использование методов и средств разработки программной документации | Swagger, XML-комментарии, `swagger.json` |

### Задание на период 22.06 – 05.07 (администрирование БД и серверов, ПМ.07)

| № | Пункт задания | Где отражено |
|---|---------------|--------------|
| 1 | Идентификация технических проблем БД | `logs/postgresql_log_2026-06-23.txt`, README (23.06) |
| 2 | Администрирование отдельных компонентов серверов | `deploy/create_roles.sql`, `deploy/backup_db.bat`, `deploy/docker-compose.prod.yml` |
| 3 | Организация взаимосвязи отдельных компонентов | Интеграция Kafka, Redis, Vault в `PP_Anna.Api` |
| 4 | Обслуживание и поддержка работы БД | Настройка логирования, бэкап, мониторинг блокировок |
| 5 | Организация безопасности сервера БД | `security-db.md` (политика безопасности) |
| 6 | Изучение архитектуры «клиент-сервер» | Описание архитектуры Web API в README и `security-db.md` |
| 7 | Обслуживание и поддержка работы современных БД и серверов | Управление контейнерами (Docker), `docker-compose.prod.yml` |
| 8 | Разработка политики безопасности SQL-сервера и БД | `security-db.md` (роли, аудит, шифрование, бэкапы, защита от инъекций) |
